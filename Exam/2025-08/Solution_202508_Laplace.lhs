Step 0 (to simplify the calculation): add short names

Laplace: f'' t - c * f' t + 4 * f t = 0, f 0 = 2, f' 0 = c
  where  c = 3*r
         r = sqrt 2

> module Solution_202508_Laplace where
> import PS  -- Power Series - could use DSLsofMath.PSDS

3a: Solve using power series

> c :: Fractional a => a
> c = 4.242640687119286 -- 3 * sqrt 2
> fs, fs', fs'' :: Fractional a => PS a
> fs'' = c*fs' - 4*fs
> fs' = integ c fs''
> fs  = integ 2 fs'
