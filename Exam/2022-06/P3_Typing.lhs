...
where P = LocMin(norm,zero) = Exists delta > 0 . Forall x in I(zero,delta) .
            x /= zero => sin(norm(x)) > sin(norm(zero)) = 0
...
We first simplify the statement:
  ...
  not(LocMin(g,zero))
= -- Def.
  not(Exists delta > 0. Forall (x,y) in I(zero,delta) .
        (x,y) /= zero => g(x,y) > g(0,0) = 0)
= -- not-Exists
