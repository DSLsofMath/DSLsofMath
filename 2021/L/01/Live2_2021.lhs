...
(study week 1, lecture 2, of the DSLsofMath course, 2021).
...
e3 = PlusI 0 (-1)
addCC :: CC -> CC -> CC
addCC (PlusI re im) (PlusI re' im') = PlusI realdel imagdel
  where  realdel = re + re'
         imagdel = im + im'
data CC where
  PlusI :: REAL -> REAL -> CC
...
eval :: CE -> CC  -- x :: CE, jag behöver typen CC
eval (Add x y) = addCC (eval x) (eval y)
eval I         = iCC
eval (RCon r)  = rconCC r
-- semantiska "kontruerare"
-- addCC  :: CC -> CC -> CC
mulCC   :: CC -> CC -> CC
rconCC  :: REAL -> CC
iCC     :: CC
...
iCC = PlusI 0 1 -- realdel = 0, imaginärdel = 1
