...
-- TODO: convert to Literate Haskell
...
-- running the tests.
--
-- In general, any instance of Num should work though if they satisfy the
-- type class laws for Num. Note that Double does not satisfy this, due
-- to rounding issues.

-- ^ Checks that :+: is commutative for integers.
checkComm :: Integer -> Integer -> Bool
checkComm a b =
  evalSR (Var "a" :+: Var "b") env == evalSR (Var "b" :+: Var "a") env
  where env = [("a", a), ("b", b)]
...
checkDistribR a b c =
  evalSR ((va :+: vb) :*: vc) env == evalSR ((va :*: vc) :+: (vb :*: vc)) env
  where env = [("a", a), ("b", b), ("c", c)]
        va = Var "a"
        vb = Var "b"
        vc = Var "c"
