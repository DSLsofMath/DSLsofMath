...
%  possible to use a name as an infix operator.
  ...
  \item Consider the following mathematical expression with variables:
    ...
    \begin{enumerate}
      %
    \item Update the |Exp| data type with a new constructor |Var
      String| that allows variables with strings as names to be
      represented.
      %
      Use the updated |Exp| to write an expression for |c1| in
      Haskell.
      %
    \item Create a function |varVal :: String -> Integer| that takes a
      variable name, and returns the value of that variable.
      %
      For now, the function just needs to be defined for the variables
      in the expression above, i.e. |varVal "x"| should return 5,
      |varVal "y"| should return 8, and |varVal "z"| should return 13.
      %
