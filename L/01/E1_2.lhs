\begin{exercise}
%
We will now look at a more general version of the |Exp|
type from the previous exercise:
%
\begin{code}
data E2 a  =  Con a
           |  Var String
...
%
For instance, we could have an |E2 Double| to represent expression trees
with doubles at the leaves, or an |E2 ComplexD| to represent expression trees with
complex numbers at the leaves.
%
\begin{enumerate}
%
    ...
    \begin{enumerate}
        ...
        \item Create a function |varVal :: Table a -> String -> a|
          ...
          variable name. For instance, |varVal vars "d"| should return
          7.4
        \item Create a function |eval :: Num a => Table a -> E2 a ->
          a| that takes a value of the new |E2| data type and returns
          the corresponding number. For instance, |eval vars (Plus
          (Con 2) (Var "a")) == 3.5|. Try it on the expressions from
          the first part, and verify that it works as expected.
