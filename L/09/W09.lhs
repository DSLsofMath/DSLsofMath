...
therefore we will use descriptive names instead. While the concept of
a space of events underpins modern understandings of probability
theory, textbooks sometimes give a couple of examples involving coin
tosses and promptly forget the concept of sample space in the body of
the text. Here we will instead develop this concept using our DSL
methodology.  Once this is done, we will be able to see that an
accurate model of the sample space is an essential tool to solve
probability problems.

Our first task is to describe the possible structure of sample spaces,
and model them as a DSL.
To this end we will use a data type to represent spaces. This type is indexed by the
...
Finite :: [a] -> Space a
\end{spec}
The scenario (or ``experiment'') corresponding to throwing a die is then represented by
the following space:
\begin{code}
die :: Int -> Space Int
die n = Finite [1..n]
...
probability (or probability mass) in the space, and this is what we have modelled above. But this is not always
...
density) is given by a real number.
...
As a first instance of a dependency, we introduce the product of two spaces, as follows:
\begin{code}
prod :: Space a -> Space b -> Space (a,b)
\end{code}

For example, the ``experiment'' of throwing two 6-faced dice is
represented as follows:\footnote{The use of a pair corresponds to the
  fact that the two dice can be identified individually.}
%
\begin{code}
...
\end{code}
%
But let's say now that we know that the sum is greater than 7. We can then
...
of |twoDice|. To support this dependency we need a generalisation of the
...
about another important notion in probability theory: that of a distribution.
%
...
%format mu = "\mu"
%format sigma = "\sigma"
Finally we can define the normal distribution with average |mu| and standard deviation |sigma|.
\begin{code}
...
variables''. However we consider this terminology to be misleading ---
...
%
The above definition relies on the usual notions of sum (|bigsum|) and |integral|.
%
...
The simplest quantity that we can compute using the integrator is the measure of the space --- it total ``mass'' or ``volume''.
%
...
The proof proceeds by structural induction over |s|. The hypothesis of
linearity is used in the base cases. Notably the linearity property of
|Finite| and |RealLine| hinge on the linearity of sums and integrals.
The case of |Project| is immediate by definition. The case for |Sigma|
...
by any value without ``side effects'' on other variables (unlike positions (\(q\))
and velocities (\(v\))).
...
This may be quite confusing at this stage. What are those expressions,
and where do the experiment occur in the variable?  Our answer is to
using spaces to represent the ``experiments'' that Grinstead and Snell
mention. More specifically, if |s : Space a|, each possible situation
at the end of the experiment is representable in the type |a| and the
space will specify the mass of each of them (via the integrator).

Then, a |b|-valued random variable |f| related to an experiment
represented by a space |s : Space a| is a function |f| of type |a ->
b|. Then |f x| is the ``expression'' that Grinstead and Snell refer
to. The variable |x| is the outcome, and |s| is represents the
experiment --- which is most often implicit in a random variable
expressions as written in a math book.
...
value of a real-valued random variable |t|. As just mentioned, from
our point of view, this notation can be confusing because it leaves
the space of situations completely implicit. That is, it is not clear
how |t| depends on the outcome of experiments.
...
is implicit as well as the dependency between |e| and |s|.
