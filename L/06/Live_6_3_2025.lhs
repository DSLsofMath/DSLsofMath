...
mulDS (DS fs) (DS gs) = DS (mulDL fs gs)

mulDL :: Ring a => [a] -> [a] -> [a]
mulDL fs [] = []
mulDL [] gs = []
mulDL fs@(f0:fs') gs@(g0:gs') = m0 : ms'   -- [(f*g) 0, (f*g)' 0, (f*g)'' 0, ...]
  where m0 = f0*g0
        ms' = addDL (mulDL fs gs') (mulDL fs' gs)

instance Additive a => Additive (DS a) where (+) = addDS; zero = zeroDS
zeroDS = DS []
addDS (DS fs) (DS gs) = DS (addDL fs gs)
instance Ring a => Multiplicative (DS a) where (*) = mulDS; one = oneDS
...
pythagorean = sinX * sinX + cosX * cosX    -- sin² + cos² = one
