...
Code continued from earlier weeks' live coding.
...
Specification of integ:
  ...
  forall cs. integP (val0 cs) (derP cs) = cs
...
propDerInteg :: (Eq a, Field a) => a -> Poly a -> Bool
propDerInteg c  cs  =  derP (integP c cs) == cs
propIntegDer :: (Eq a, Field a) => PS a -> Bool
propIntegDer    cs  =  integP (val0 cs) (derP cs) == cs
\end{code}
