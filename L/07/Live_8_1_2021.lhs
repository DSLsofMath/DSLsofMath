...
dot ::  (Finite a, Ring s) =>
        Vec s a -> Vec s a -> s
dot v w = sum (map prod finiteDomain)
  where prod i = v i * w i

mulMatVec ::  (Finite c, Finite r, Ring s) =>
              Mat s c r -> Vec s c -> Vec s r
mulMatVec m v = \i -> dot (m i) v
  -- m   :: Mat s c r = r -> Vec s c
  -- v   :: Vec s c   =      c -> s
  -- rhs :: Vec s r   =      r -> s
  -- rhs = \i ->
  -- if  i :: r  then  (m i) :: Vec s c
